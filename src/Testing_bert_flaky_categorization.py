...
print('checking cuda available torch version******************************')
...
def parse_cr(cr, ml_technique, fold, feature_extraction_time, prediction_time, fit_time):
    ...
    with open("../flaky-test-categorization/weighted_avg_for_cv_"+technique+".txt", "a") as file: # Once I get the result, need to divide by 10
        file.write(fold+",")
...
for fold in range(no_split):
    ...
    time_to_quantize=time.time() - start_time
    print("**time_to_quantize=",time_to_quantize)
